...
| **Wire Contract / Nullable Drift** | Medium | <e.g. the Go handler and the `@lib/contracts` type agree; optional fields guarded on the client> |
...
<e.g. cd modules/apps/mobile && npx vitest run <target>.test.ts, then the full gate for the layer touched>
```
