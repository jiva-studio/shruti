...
The conventions for Vue 3 and TypeScript in shruti: the mobile app under `modules/apps/mobile` (Vue 3 + Ionic + Capacitor), the shared `modules/libs/ui`, and the `@kit/ui` primitives in the `modules/kit` submodule. The architecture these serve is in [`architecture.md`](./architecture.md) and, in full, in [`docs/repos/shruti/architecture/layers.md`](../../docs/repos/shruti/architecture/layers.md).
