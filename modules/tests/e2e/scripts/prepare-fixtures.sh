...
REPO_ROOT="$(cd "$HERE/../../.." && pwd)"                     # …/lectorium
SHOTS="$REPO_ROOT/modules/tools/screenshots"
FIX="$HERE/fixtures"
mkdir -p "$FIX"

# content.db: local lake → screenshot fixture → published CDN (CI path).
PUBLIC_BASE="${LECTORIUM_PUBLIC_BASE:-https://akds-lectorium.s3.us-east-1.amazonaws.com}"
...
if [[ -f "$CATALOG" ]]; then
  ...
  curl -fsSL "$PUBLIC_BASE/public/db/lectorium.$VER.db" -o "$FIX/content.db"
fi
