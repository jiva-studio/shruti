...
func RegisterPipelineRun(s *server.MCPServer, deps Deps) {
	...
	tool := mcp.NewTool(kind,
		...
		mcp.WithString("op", mcp.Description("Operation: pipeline (default) | audio_tag | align_pdf | audit | titles_refresh | outline.")),
	...
	s.AddTool(tool, func(ctx context.Context, req mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		if !pipeline.IsValidOp(op) {
			return envelope.Err(kind, envelope.CodeInvalidArgument,
				fmt.Sprintf("op %q invalid; allowed: pipeline | audio_tag | align_pdf | audit | titles_refresh | outline", op),
				nil), nil
		}
