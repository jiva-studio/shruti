...
func registerDenoiseWait(s *server.MCPServer, p Provider, cfg Config) {
	tool := mcp.NewTool("denoise_wait",
		...
		mcp.WithString("strategy",
			mcp.Description("Cleaning strategy: afftdn (default) | rnnoise | rnnoise-mix | afftdn-rnnoise-mix.")),
		mcp.WithNumber("nr",
...
func registerDenoiseBatch(s *server.MCPServer, p Provider) {
	tool := mcp.NewTool("denoise_batch",
		...
		mcp.WithString("strategy", mcp.Description("Cleaning strategy: afftdn (default) | rnnoise | rnnoise-mix | afftdn-rnnoise-mix.")),
