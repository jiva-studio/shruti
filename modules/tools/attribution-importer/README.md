...
YAML file via the shruti-mcp HTTP endpoint. Idempotent, resumable,
single-purpose.
...
HTTP transport that shruti-mcp serves). Sends one `tools/call`
request per attribution-create / ref-add. No SSE, no streaming — every
call is one round-trip.

Default daemon address is read from `shruti-mcp.yaml` in the daemon's
