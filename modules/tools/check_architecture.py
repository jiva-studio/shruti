#!/usr/bin/env python3
"""Universal Monorepo Architecture & Layer Guard for Lectorium.
...
class ArchitectureScanner:
    ...
    def _check_ts_import(self, file_path: Path, line_no: int, rel: str, target: str) -> None:
        ...
        if "modules/libs/domain/" in rel:
            if any(target.startswith(b) for b in ["@usecases", "@ports", "@infra", "@ui", "@lectorium", "vue", "@ionic"]):
        ...
        # Rules for @usecases (modules/apps/mobile/usecases)
            ...
            if any(target.startswith(b) for b in ["@ports", "@infra", "@ui", "@lectorium", "@lib/persistence", "vue", "@ionic"]):
                self.violations.append(
                    Violation(
                        file_path=file_path,
                        line_number=line_no,
        ...
        # Rules for @ui (modules/apps/mobile/ui)
            ...
            if any(target.startswith(b) for b in ["@ports", "@infra", "@lib/domain", "@usecases", "@lectorium"]):
                self.violations.append(
                    Violation(
                        file_path=file_path,
                        line_number=line_no,
        ...
        # Rules for @lib/ui (modules/libs/ui - Shared UI)
            ...
            if any(target.startswith(b) for b in ["@ports", "@infra", "@lib/domain", "@lib/catalog", "@lib/contracts", "@lib/persistence", "@usecases", "@lectorium", "@ionic"]):
                self.violations.append(
                    Violation(
                        file_path=file_path,
                        line_number=line_no,
        ...
        # Rules for @ports/app (modules/apps/mobile/ports)
            ...
            if any(target.startswith(b) for b in ["@lib/domain", "@usecases", "@infra", "@ui", "@lectorium"]):
                self.violations.append(
                    Violation(
                        file_path=file_path,
                        line_number=line_no,
        ...
        # Rules for persistence row types (@lib/persistence)
            ...
            if any(target.startswith(b) for b in ["@lib/domain", "@usecases", "@infra", "@ports", "@ui", "@lectorium"]):
                self.violations.append(
                    Violation(
                        file_path=file_path,
                        line_number=line_no,
    ...
    def _scan_python_chat(self) -> None:
        chat_src = self.repo_root / "modules" / "services" / "chat" / "app" / "src" / "lectorium_chat"
