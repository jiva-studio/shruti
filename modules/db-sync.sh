...
CDN_URL="https://akds-lectorium.s3.us-east-1.amazonaws.com"
...
mkdir -p "${CACHE_DIR}"
CACHED_DB="${CACHE_DIR}/lectorium.${VERSION}.db"
CACHED_CONFIG="${CACHE_DIR}/config.json"

if [ -f "${CACHED_DB}" ]; then
  echo "[db-sync] Using cached DB: ${CACHED_DB}"
else
  echo "[db-sync] Downloading ${CDN_URL}/public/db/lectorium.${VERSION}.db..."
  curl -fSL --progress-bar "${CDN_URL}/public/db/lectorium.${VERSION}.db" -o "${CACHED_DB}.part"
...
distribute_to_dir() {
  ...
  local label="$2"
  mkdir -p "${dir}"
  # Remove any stale lectorium.*.db in the target so lexicographic pickup
  # never returns an old file after a scheme bump.
  find "${dir}" -maxdepth 1 -type f -name 'lectorium.*.db' ! -name "lectorium.${VERSION}.db" -delete
  cp -f "${CACHED_DB}" "${dir}/lectorium.${VERSION}.db"
  echo "[db-sync] ${label}: ${dir}/lectorium.${VERSION}.db"
}
