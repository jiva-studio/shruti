...
// override grant Pro at all (see lectorium/services/devSubscription.ts), and it
...
const e2eBuild = process.env.LECTORIUM_E2E_BUILD === "1"
...
const sentryRelease = commitSha
  ? `lectorium@${pkg.version}+${commitSha}`
  : `lectorium@${pkg.version}`
...
export default defineConfig({
  define: {
    ...
    // empty string disables Sentry (see lectorium/services/monitoring).
    __SENTRY_DSN__: JSON.stringify(
      process.env.SENTRY_DSN ??
        "https://3700c109f2dbe903462bfa828cfce5e4@o257342.ingest.us.sentry.io/4511584811220992"
    ),
    ...
    // LECTORIUM_OFFSTORE is set, the web bundle hides Google/Apple sign-in
    ...
    __OFFSTORE_BUILD__: JSON.stringify(
      process.env.LECTORIUM_OFFSTORE === "1" || process.env.LECTORIUM_OFFSTORE === "true"
    ),
  ...
  plugins: [
    ...
    ...(e2eBuild
      ? [
          {
            name: "lectorium-e2e-build-marker",
    ...
    ...(process.env.SENTRY_AUTH_TOKEN
      ? [
          sentryVitePlugin({
            ...
            project: process.env.SENTRY_PROJECT ?? "letorium",
