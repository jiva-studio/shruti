...
const TEMPLATE = "lectorium/databases/lectorium.{version}.db"
const DIR = "lectorium/databases"
...
describe("sweepCatalogCopyTemps", () => {
  it("deletes copy temps and leaves every real catalog alone", async () => {
    const s = store([
      `${DIR}/lectorium.20260101000000.db`,
      `${DIR}/lectorium.20260101000000.db.copying`,
      `${DIR}/lectorium.20260202000000.db`,
    ...
    expect(swept).toEqual([`${DIR}/lectorium.20260101000000.db.copying`])
    expect(s.deleted).toEqual([`${DIR}/lectorium.20260101000000.db.copying`])
  })
  ...
  it("sweeps a temp whose final catalog is long gone", async () => {
    ...
    const s = store([
      `${DIR}/lectorium.20260303000000.db`,
      `${DIR}/lectorium.20260101000000.db.copying`,
    ])
