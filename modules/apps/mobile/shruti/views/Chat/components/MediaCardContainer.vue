...
const posterUrl = computed<string | undefined>(() => {
  ...
  // No extension in the last segment: the swap is a no-op, and a poster
  // pointing at the video decodes as nothing at all. Better no poster — the
  // browser draws the first frame once metadata is in.
  return poster === url ? undefined : resolve(poster)
})
