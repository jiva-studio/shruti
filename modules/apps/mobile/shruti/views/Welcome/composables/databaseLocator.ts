...
/**
 * Scans the local database directory for an existing shruti DB file.
 * Works offline — no network required. Returns the storage path
 * (e.g. "shruti/databases/shruti.20260411211018.db") or null.
...
export async function findLocalDatabase(
  ...
  // dir = "shruti/databases/shruti" → parent = "shruti/databases"
  ...
  const dbPath = files
    .filter((path) => {
      ...
      return name.startsWith("shruti.") && name.endsWith(".db")
    })
