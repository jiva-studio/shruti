export default {
  youMightLike: "Może Ci się spodobać",
  untitled: "Wykład bez tytułu",
  remove: "Usuń z Mojej biblioteki",
  status: {
    processing: "Przetwarzanie",
    ready: "Gotowe",
    failed: "Niepowodzenie",
    retry: "Ponów",
    stages: {
      downloading: "Pobieranie",
      transcribing: "Transkrypcja",
      reviewing: "Korekta",
      storing: "Zapisywanie",
    },
    errors: {
      unauthorized: "Dodawanie wykładów to funkcja Pro.",
      unavailable: "To źródło jest niedostępne.",
      unsupported: "Tego linku nie można dodać.",
      too_large: "Ten wykład jest zbyt długi, aby go dodać.",
      no_speech: "Nie znaleziono mowy do transkrypcji.",
      internal: "Coś poszło nie tak.",
    },
  },
  myLibrary: {
    title: "Moja biblioteka",
    seeAll: "Zobacz wszystkie wykłady w mojej bibliotece",
    emptyTitle: "Twoja biblioteka jest pusta",
    emptyMessage: "Poproś Sadhu, aby znalazł wykład w internecie i dodał go do twojej biblioteki.",
    loadError: "Nie udało się wczytać biblioteki. Pociągnij w dół, aby odświeżyć.",
  },
}
