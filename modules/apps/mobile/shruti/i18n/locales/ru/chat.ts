export default {
  ...
  proactiveSmartLibraryHintBody:
    "Ты дослушиваешь лекции до конца — умная библиотека может держать в офлайне свежую очередь по твоим темам. Попробуешь?",
  proactiveEnableNotificationsBody:
    "Ты слушаешь несколько дней подряд — хочешь, я буду присылать напоминание, чтобы не сбить ритм? Поставлю утреннее время, его можно поменять в настройках.",
