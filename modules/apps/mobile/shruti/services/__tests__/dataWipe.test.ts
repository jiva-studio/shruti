...
describe("wipeLocalUserData", () => {
  beforeEach(async () => {
    ...
    app = {
      ...
      databaseFetcher: {
        list: async () => ["shruti.7.db", "shruti.42.db", "user.db"],
  ...
  it("deletes the content catalog but not the user DB file (#1630)", async () => {
    ...
    expect(deletedDbPaths).toEqual([
      "shruti/databases/shruti.7.db",
      "shruti/databases/shruti.42.db",
    ])
  ...
  it("deletes the catalog on the WEB build too, where nothing could list it", async () => {
    ...
    idb.set("shruti/databases", blobs)
    const webApp = { ...app, databaseFetcher: useDatabaseToIndexedDbFetcher() } as Shruti
