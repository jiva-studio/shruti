...
import { findRegion } from "@shruti/services/regionsRegistry.js"
  ...
  function trigger(opts?: { force?: boolean }): void {
    ...
    void deps
      ...
      .catch((err) => {
        ...
        console.warn("[shruti] region re-probe failed:", err)
      })
      .finally(() => {
        inFlight = false
      })
  ...
  const reprobe = createRegionReprobe({
    probe: async () => {
      const result = await shruti.serverProber.probe(
        shruti.appConfig.publicRemoteConfigPath,
        shruti.activeServer.value.id
      )
    ...
    onResolved: (serverId) => {
      ...
      shruti.setActiveServerById(serverId)
    },
  })
