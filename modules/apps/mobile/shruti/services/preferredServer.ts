...
export async function promotePreferredServer(
  shruti: Pick<Shruti, "activeServer" | "setActiveServer" | "preferences">,
  server: CdnServer
): Promise<void> {
  if (shruti.activeServer.value.id === server.id) return
  shruti.setActiveServer(server)
  try {
    await shruti.preferences.set(PREFERRED_SERVER_KEY, server.id)
  } catch (err) {
