...
# SHRUTI_GOOGLE_IOS_CLIENT_ID env var as __GOOGLE_IOS_CLIENT_ID__).
#
...
# only does something when SHRUTI_GOOGLE_IOS_CLIENT_ID overrides that
...
CLIENT_ID="${SHRUTI_GOOGLE_IOS_CLIENT_ID:-}"

# No override → committed default stands. Nothing to do.
if [[ -z "$CLIENT_ID" ]]; then
  echo "[ios-google-signin] SHRUTI_GOOGLE_IOS_CLIENT_ID unset — keeping Info.plist default"
  exit 0
fi
