...
export default defineConfigWithVueTs(
  ...
  // Only the composition root binds an adapter. Type-only imports are fine.
  {
    files: [
      "lectorium/stores/**/*.ts",
      "lectorium/composables/**/*.ts",
      "lectorium/services/**/*.ts",
      "lectorium/proactive/**/*.ts",
    ],
    // bootstrap.ts is startup wiring — composition root in all but location.
    ignores: ["**/__tests__/**", "**/*.test.ts", "lectorium/services/bootstrap.ts"],
    rules: {
      ...
      "@typescript-eslint/no-restricted-imports": [
        "error",
        {
          patterns: [
            ...
            { group: ["@lectorium/*"], message: "@lib/chat must not import composition root" },
            {
              group: ["@capacitor/*"],
  ...
  // what it is describing and is not held to either.
  {
    ...
    ignores: ["**/*.test.ts", "**/__tests__/**", "lectorium/i18n/**"],
    rules: {
