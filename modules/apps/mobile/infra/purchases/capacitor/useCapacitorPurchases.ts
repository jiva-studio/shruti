...
async function toCustomerState(info: CustomerInfo): Promise<CustomerState> {
  ...
  if (activeEnt) {
    try {
      ...
      const pkg = offerings.current?.availablePackages.find(
        (p) => p.product.identifier === activeEnt.productIdentifier
      )
      activePackageId = pkg?.identifier
    } catch {
      // Offerings unavailable — UI shows the generic "active" state.
    }
  }
  ...
  return {
    activePackageId,
    activeEntitlements,
    managementUrl: getManagementUrl(activeEnt?.productIdentifier),
  }
}
