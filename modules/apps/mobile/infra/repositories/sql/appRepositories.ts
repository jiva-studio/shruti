...
export interface SqlAppRepositories {
  ...
  /** Read-only; rows are pull-only from `profile`. Always present — it does
   *  not depend on the sync `getDeviceId` gate, and the sync-apply adapter
   *  fills the table when the engine runs. */
  readonly libraryItems: ReturnType<typeof createSqlLibraryItemRepository>
  /** The user's remove/re-add intent for library items: CLIENT-owned and
   *  synced, wrapped by the journal decorator when the sync engine is enabled.
   *  The store joins it against `libraryItems` to hide removed items. */
  ...
  /**
   * Profile-sync engine repositories. Present only when `getDeviceId` is
   * wired — the same gate that enables journaling — so `undefined` on web and
   * before the engine is enabled. The composable is the runtime gate; these
   * are the ports it hands the engine.
   */
  ...
  /**
   * First-sync backfill reader, under the same `getDeviceId` gate as the
   * engine repos. Enumerates pre-journaling local rows so `backfillLocal` can
   * enqueue them the first time a real account signs in on this device.
   */
...
export interface CreateSqlAppRepositoriesDeps {
   ...
   * that replaces it. Omit to journal rows unowned.
   */
 ...
 * transcripts) are wired in the composition root (`shruti/`),
...
export function createSqlAppRepositories(deps: CreateSqlAppRepositoriesDeps): SqlAppRepositories {
  ...
  const baseSynced = {
    ...
    // transaction happened to be open and vanish with its rollback.
    ...
    // Its own PLAIN unit of work: the chat-message `meta` read-modify-writes
    // must be isolated from every other transaction. The two instances behave
    // identically for a caller that passes no handle, so this can move to the
    // shared one — left for a change of its own.
  ...
  return {
    ...
    // passes the handle into `upsert`, which only the instance that opened it
    // can recognise as a join.
