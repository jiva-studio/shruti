...
/** Makes an exported sql.js image durable — IndexedDB in the app, a plain
 *  buffer in tests. */
...
export function createSqlJsDatabase(db: Database, persist: PersistSink): IDatabase {
  ...
  // Whether any statement has run since the last export. On the native
  // adapter COMMIT *is* durability, so its repositories write with raw
