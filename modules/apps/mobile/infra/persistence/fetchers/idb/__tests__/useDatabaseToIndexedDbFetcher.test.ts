...
const PATH = "shruti/blobs/content.db"
...
describe("useDatabaseToIndexedDbFetcher — download", () => {
  ...
  it("lets a second fetcher download while the first one is busy", async () => {
    ...
    await useDatabaseToIndexedDbFetcher().download("https://cdn/db", "shruti/blobs/user.db")
...
describe("useDatabaseToIndexedDbFetcher — cached blobs", () => {
  beforeEach(() => {
    store.saved = []
    store.deleted = []
    store.keys = new Map([["shruti/blobs", ["content.db.20", "user.db"]]])
    store.listFailure = null
  })
  ...
  it("reports a cached key as present and an uncached one as absent", async () => {
    ...
    expect(await fetcher.exists("shruti/blobs/user.db")).toBe(true)
    expect(await fetcher.exists("shruti/blobs/content.db.21")).toBe(false)
  })
  ...
  it("removes the addressed key", async () => {
    await useDatabaseToIndexedDbFetcher().delete("shruti/blobs/content.db.20")

    expect(store.deleted).toEqual(["shruti/blobs/content.db.20"])
  })

  it("lists cached blobs as full paths the other methods accept", async () => {
    const fetcher = useDatabaseToIndexedDbFetcher()

    const paths = await fetcher.list("shruti/blobs")

    expect(paths).toEqual(["shruti/blobs/content.db.20", "shruti/blobs/user.db"])
  ...
  it("lists nothing for a directory that names no store", async () => {
    ...
    expect(await fetcher.list("shruti")).toEqual([])
  ...
  it("lists nothing rather than failing startup when the store cannot be opened", async () => {
    ...
    expect(await useDatabaseToIndexedDbFetcher().list("shruti/blobs")).toEqual([])
  })
})
