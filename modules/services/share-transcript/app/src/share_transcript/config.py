...
def load() -> Settings:
    bucket = _first("SHRUTI_S3_BUCKET", "BUCKET")
    if not bucket:
        raise RuntimeError("SHRUTI_S3_BUCKET (or BUCKET) is required")
    endpoint = _env("S3_ENDPOINT_URL")
    public_base = _first("PDFS_PUBLIC_BASE", "SHRUTI_S3_PUBLIC_BASE")
    ...
    if endpoint and not public_base:
        raise RuntimeError(
            ...
            "SHRUTI_S3_PUBLIC_BASE is empty — returned PDF URLs would "
