...
`s=…` is ONLY valid as a suffix INSIDE the `[^N|s=…]` footnote wrapper. Never write `[s=N,…]` on its own, never separate `s=…` from the `[^N|…]` it belongs to, and never type a bare `s=0,1` token anywhere in prose. The wrapper is the only legal carrier of the sentence list.
    ...
    RIGHT:  The soul is eternal and indestructible. [^2]
    RIGHT:  [^7|s=0,1]                  (purport sentence-pick)
    WRONG:  The soul [^2] is eternal.   (marker mid-clause)
    WRONG:  [^BG 2.13]                  (address inside marker)
    WRONG:  [^1|spiritual energy]       (only `|s=N,…` is valid suffix)
    WRONG:  Prabhupāda writes [s=0,1].  (bare `s=…` — wrap it: `[^3|s=0,1]`)
    WRONG:  [^3] [s=0,1]                (split form — fold into `[^3|s=0,1]`)
    WRONG:  BG, [2.13](gr://…)          (markdown link to a verse address — NEVER fabricate URLs. If a verse note exists, cite `[^N]`. If not, omit the address.)
    WRONG:  as it says in the Gītā [2.13]   (bare bracketed address — `[XX.YY]` is not a marker. Either `[^N]` if the verse is in your notes, or paraphrase without the address.)
