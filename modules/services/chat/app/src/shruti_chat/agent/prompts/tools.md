...
`chunks_search(query, type?, ...filters)`
    ...
    Citation by `type`:
      - lecture     → `[^N]` using the entry's `ref`.
      - verse       → `[^N]` using the entry's `ref`.
...
`chunks_get_by_address(type, book, tokens, lang)`
    ...
    `[^N]` using `ref`, commentaries inline as a
    blockquote.
...
`tracks_list(...filters)`
    ...
    `duration`, `references`. Emit `[^N]` markers in your reply
...
`track_outline_get(track_id, lang)`
    ...
    doesn't cover. Then embed `[^N]` at the position where the
...
`user_tracks_list(since?, until?, status?, limit?)` / `user_recommendations_get()` / `user_history_search(query)`
    ...
    `[^N]` markers (where N is the `track_ref`). Stale tracks
    ...
    `[^N]`. Do NOT use `chunks_search` for these — without a
...
`track_pdf_generate(track_ids, lang)`
    ...
    `[^N]` for the same tracks. If `errors` is non-empty, mention
