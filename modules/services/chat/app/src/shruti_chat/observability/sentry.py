...
already reports into the same `akdasa-studio/shruti` project — so once the
...
def _structlog_payload(hint: dict[str, Any]) -> dict[str, Any] | None:
    ...
    Safe to hoist wholesale into `extra`: `drop_pii` is a shared processor, so
    this dict has already been through it by the time it becomes `record.msg`.
    """
