"""Trust-boundary helper for the `X-Lectorium-Region` header.

The RU proxy (#728) tags every forwarded request with
`X-Lectorium-Region: ru` so the global backend can gate PII-class
...
# untrusted and the X-Lectorium-Region header is silently ignored.
...
def extract_region(request: Request) -> str | None:
    """Return the `X-Lectorium-Region` header iff the request source is trusted.
    ...
    """
    raw = request.headers.get("X-Lectorium-Region")
    if not raw:
        return None
