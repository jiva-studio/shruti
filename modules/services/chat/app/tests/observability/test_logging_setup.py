...
from shruti_chat.observability import logging as obs_logging
...
def test_foreign_stdlib_records_carry_the_service_label(isolated_logging, capsys):
    ...
    assert len(out) == 2
    for line in out:
        assert json.loads(line)["service"] == "shruti-chat"
...
def test_the_service_label_survives_a_context_switch(isolated_logging, capsys):
    ...
    assert _last_json_line(capsys.readouterr().out)["service"] == "shruti-chat"
...
def test_client_ip_hash_uses_the_configured_salt(monkeypatch):
    ...
    from shruti_chat import config as cfg
