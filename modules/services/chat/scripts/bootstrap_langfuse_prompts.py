...
Reads:
- All `.md` files under `app/src/shruti_chat/agent/prompts/`
...
- The hardcoded `_ROUTER_SYSTEM_PROMPT` from
  `app/src/shruti_chat/application/router_turn.py`
...
def _load_md(name: str) -> str:
    path = (
        _repo_root() / "src" / "shruti_chat" / "agent" / "prompts" / f"{name}.md"
    )
    if not path.exists():
...
def _load_router_constant() -> str:
    ...
    from shruti_chat.application.router_turn import _ROUTER_SYSTEM_PROMPT
