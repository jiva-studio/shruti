// Package store holds the orchestrator's Postgres pool and embedded migrations.
//
// Like profile (and unlike auth/chat, which use the central `migrator`), the
// orchestrator carries its OWN embedded migrations and runs them one-shot
// before serving — it fully owns its database (the `jobs` table is the source
// of truth for the ingest pipeline).
package store
...
const migrateAdvisoryLockKey = 0x6F72636865734D // "orchesM" bytes, fits int64

// Connect opens a pool. Queries qualify objects with `orchestrator.<table>` so
// any session works regardless of search_path.
...
func Migrate(ctx context.Context, pool *pgxpool.Pool) error {
	...
	// The bookkeeping table lives in the orchestrator schema, which the first
	// migration also creates — so create the schema + ledger defensively here.
	if _, err := conn.Exec(ctx, `CREATE SCHEMA IF NOT EXISTS orchestrator`); err != nil {
		return fmt.Errorf("create schema: %w", err)
	}
	if _, err := conn.Exec(ctx, `CREATE TABLE IF NOT EXISTS orchestrator.schema_migrations (
	...
	for _, name := range names {
		...
		if err := conn.QueryRow(ctx,
			`SELECT EXISTS (SELECT 1 FROM orchestrator.schema_migrations WHERE version = $1)`,
			name,
		...
		if _, err := tx.Exec(ctx,
			`INSERT INTO orchestrator.schema_migrations (version) VALUES ($1)`, name,
		); err != nil {
			_ = tx.Rollback(ctx)
...
func SchemaReady(ctx context.Context, pool *pgxpool.Pool) error {
	...
	for _, name := range names {
		...
		if !applied {
			return fmt.Errorf("migration %s not applied — run `orchestrator migrate`", name)
		}
	}
	return nil
}
