...
Shruti catalog and posts it to social platforms. Two content kinds —
...
| `SHRUTI_BUILD_SHA` / `_TIME` | `dev` | surfaced on `/healthz` |
...
1. Provision on the host:
   - `/opt/shruti/social-poster/config.yaml` (from `config.example.yaml`,
     real channel/group/page ids + Bunny poster URLs).
   - In `.env`: `SHRUTI_TELEGRAM_BOT_TOKEN`, `SHRUTI_VK_COMMUNITY_TOKEN`,
     `SHRUTI_FB_PAGE_TOKEN` (the overlay maps these to the plain names the
