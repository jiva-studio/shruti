...
func TestHTTPFetcherResolvesLatestVersion(t *testing.T) {
	...
	srv := &manifestServer{
		...
		objects:  map[string][]byte{"public/db/lectorium.20260809133448.db": blob},
		hits:     map[string]int{},
	}
	...
	if n := srv.hits["public/db/lectorium.20260809133448.db"]; n != 1 {
...
func TestHTTPFetcherSkipsDownloadWhenVersionUnchanged(t *testing.T) {
	srv := &manifestServer{
		...
		etag:     `"cfg-v1"`,
		objects:  map[string][]byte{"public/db/lectorium.20260809133448.db": catalogBytes(t, "trk-a")},
		hits:     map[string]int{},
	}
	ts := httptest.NewServer(http.HandlerFunc(srv.handler))
	defer ts.Close()
...
func TestHTTPFetcherNotFound(t *testing.T) {
	cases := []struct {
	...
	}{
		{
			...
			manifest: `{"databases":[{"version":20260809133448}]}`,
			objects:  map[string][]byte{},
			want:     "lectorium.20260809133448.db: status 404",
		},
		{
...
func TestBlobFetcherResolvesLatestVersion(t *testing.T) {
	...
	want := []string{manifestKey, "public/db/lectorium.20260809133448.db"}
