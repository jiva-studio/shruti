...
func TestParseRemoteRegions(t *testing.T) {
	cases := []struct {
		...
		{
			name: "single",
			in:   "russia=https://auth.russia.shruti.app",
			want: []RemoteRegion{
				{ID: "russia", BaseURL: "https://auth.russia.shruti.app"},
			},
		},
		{
			name: "multiple",
			in:   "russia=https://auth.russia.shruti.app,asia=https://auth.asia.shruti.app",
			want: []RemoteRegion{
				{ID: "russia", BaseURL: "https://auth.russia.shruti.app"},
				{ID: "asia", BaseURL: "https://auth.asia.shruti.app"},
			},
		},
		{
...
// cross-region delivery (REMOTE_REGIONS without SHRUTI_INTERNAL_
...
func TestLoad_FailsWhenRemoteRegionsSetWithoutSecret(t *testing.T) {
	...
	t.Setenv("SHRUTI_INTERNAL_SECRET", "")
	if _, err := Load(); err == nil {
		t.Fatal("expected Load to fail when REMOTE_REGIONS set without SHRUTI_INTERNAL_SECRET")
	}
}
...
func TestLoad_SecretWithoutRemotesIsHarmless(t *testing.T) {
	t.Setenv("DATABASE_URL", "postgres://stub")
	t.Setenv("SHRUTI_INTERNAL_SECRET", "shared-secret")
