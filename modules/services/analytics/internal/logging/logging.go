...
// pipeline auto-parses (service / env / version / level / timestamp /
// message), plus a per-request request_id pulled off the context.
//
...
// WithRequestID returns a context carrying the request id that every
...
// plus a handler that walks the context for the request id.
