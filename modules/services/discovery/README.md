...
- `LECTORIUM_DISCOVERY_SCHEDULER_ENABLED=true` — off by default
...
fetcher. `LECTORIUM_DISCOVERY_SCHEDULER_WORKERS` says how many pages may be in flight at
...
One name, everywhere: `LECTORIUM_DISCOVERY_*` in the deployment `.env`, in
...
|---|---|---|
| `LECTORIUM_DISCOVERY_SCHEDULER_ENABLED` | `false` | the only switch that makes the service crawl on its own |
| `LECTORIUM_DISCOVERY_SCHEDULER_WORKERS` | `4` | pages in flight at once, across every source — not a rate limit |
| `LECTORIUM_DISCOVERY_PAGE_TIMEOUT` | `10m` | one page end to end: fetch, model, embed, write |
| `LECTORIUM_DISCOVERY_DB_MAX_CONNS` | `16` | pool size; raised automatically if smaller than the worker count |
| `LECTORIUM_DISCOVERY_USER_AGENT` | names us, with a contact URL | who a volunteer archive sees, and where to complain |
| `LECTORIUM_DISCOVERY_CRAWL_DELAY` | `1s` | gap between requests to one host where `robots.txt` states none |
| `LECTORIUM_DISCOVERY_REQUEST_TIMEOUT` | `30s` | one outbound request |
| `LECTORIUM_DISCOVERY_MAX_BODY_BYTES` | `8388608` | the most of one response we will read |
| `LECTORIUM_DISCOVERY_PROXY` | empty | for archives a datacenter address cannot read; YouTube answers one with a bot check |
| `LECTORIUM_DISCOVERY_LLM_BASE_URL` | `https://openrouter.ai/api/v1` | the endpoint; what decides whether the normalizer runs is the key below |
| `LECTORIUM_DISCOVERY_LLM_API_KEY` | empty | unset leaves the normalizer stubbed: pages are still fetched and stored, just not read |
| `LECTORIUM_DISCOVERY_LLM_MODEL` | `google/gemini-3.1-flash-lite` | part of the input hash, so changing it re-reads everything |
| `LECTORIUM_DISCOVERY_EMBED_MODEL` | `openai/text-embedding-3-small` | matches the corpus, so vectors stay comparable |
| `LECTORIUM_DISCOVERY_EMBED_DIM` | `1536` | must match the vector column or every insert fails |
...
export LECTORIUM_DISCOVERY_TEST_DATABASE_URL='postgresql://discovery:x@127.0.0.1:55444/discovery?sslmode=disable'
