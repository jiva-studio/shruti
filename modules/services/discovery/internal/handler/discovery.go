...
// alikeAuthorsHandler proposes people who may be one person.
//
// By the sound of the name rather than its spelling, so two romanisations meet
// and so do the two alphabets. It proposes; somebody decides, with the merge
// below. Nothing here joins anybody on its own — a rule loose enough to catch
// every spelling of one person is loose enough to join two people, and that is
// not recoverable by looking at the result.
func alikeAuthorsHandler(repo *store.Repo) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		groups, err := repo.Alike(r.Context(), intParam(r, "limit", 100))
		if err != nil {
			writeErr(w, http.StatusInternalServerError, "query_failed", err.Error())
			return
		}
...
// mergeRequest names the person who stays and the one absorbed into them.
type mergeRequest struct {
	Keep   int64 `json:"keep"`
	Absorb int64 `json:"absorb"`
}

// mergeAuthorsHandler makes two rows one person.
//
// The spellings move, not the recordings, so the next crawl resolves the
// absorbed name to whoever survived rather than recreating the row just
// removed. That is what the spellings table is for.
func mergeAuthorsHandler(repo *store.Repo) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if req.Keep <= 0 || req.Absorb <= 0 {
			writeErr(w, http.StatusBadRequest, "bad_request", "keep and absorb are required")
			return
		}
		if req.Keep == req.Absorb {
			writeErr(w, http.StatusBadRequest, "bad_request", "keep and absorb are the same person")
			return
		}
		if err := repo.MergeAuthors(r.Context(), req.Keep, req.Absorb); err != nil {
			writeErr(w, http.StatusInternalServerError, "merge_failed", err.Error())
			return
		}
		authors, err := repo.Authors(r.Context(), "", 1000)
		if err != nil {
		...
		for _, a := range authors {
			if a.ID == req.Keep {
				writeJSON(w, http.StatusOK, map[string]any{"merged": true, "author": a})
				return
			}
		}
		writeJSON(w, http.StatusOK, map[string]any{"merged": true})
