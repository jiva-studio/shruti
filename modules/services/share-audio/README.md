...
- Bucket name (`akds-lectorium`) is hardcoded in `serverless-aws.yml`. No repo vars required for AWS.

### YC deploy — secrets/vars still to add

- **Repo secrets**:
  - `YC_OAUTH_TOKEN`, `YC_SERVICE_ACCOUNT_ID` — YC deploy + runtime SA.
  - `AWS_RUNTIME_ACCESS_KEY_ID`, `AWS_RUNTIME_SECRET_ACCESS_KEY` — dedicated IAM user with `s3:GetObject` + `s3:PutObject` + `s3:PutObjectAcl` on `akds-lectorium`. Used by boto3 inside the YC function (YC service accounts can't sign AWS S3 requests).
- **Repo vars**:
