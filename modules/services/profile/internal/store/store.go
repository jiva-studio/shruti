...
// migrateAdvisoryLockKey is an arbitrary constant guarding concurrent
// `profile migrate` runs (two one-shot containers racing on boot). Serializes
// migration application across the fleet.
const migrateAdvisoryLockKey = 0x70726F66696C654D // "profileM" bytes, fits int64

// Connect opens a pool. Queries qualify objects with `profile.<table>` so any
...
func Migrate(ctx context.Context, pool *pgxpool.Pool) error {
	...
	if _, err := conn.Exec(ctx, `CREATE SCHEMA IF NOT EXISTS profile`); err != nil {
		return fmt.Errorf("create schema: %w", err)
	}
	if _, err := conn.Exec(ctx, `CREATE TABLE IF NOT EXISTS profile.schema_migrations (
...
func SchemaReady(ctx context.Context, pool *pgxpool.Pool) error {
	...
	for _, name := range names {
		...
		if !applied {
			return fmt.Errorf("migration %s not applied — run `profile migrate`", name)
		}
	}
	return nil
}
