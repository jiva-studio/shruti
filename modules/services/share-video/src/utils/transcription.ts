 ...
 * `LECTORIUM_OPENAI_API_KEY` (the unified name used end-to-end). We do NOT
...
export async function transcribeAudioWithTimestamps(
  ...
  const key = options.apiKey || process.env.LECTORIUM_OPENAI_API_KEY;
  if (!key) {
    throw new Error(
      'OpenAI API key missing — set LECTORIUM_OPENAI_API_KEY or pass apiKey',
    );
  }
