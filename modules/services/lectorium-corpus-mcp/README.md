...
Public, **read-only** MCP server over the Lectorium scripture + lecture corpus.
Tools surface as `mcp__lectorium-corpus-mcp__*`. It reads existing data only —
...
catalog (`public/db/lectorium.{v}.db`) and library (`public/library/library.{v}.db`),
