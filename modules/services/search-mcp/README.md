...
`shruti-mcp`'s `library.attribution.*` tools.

It is the read/search counterpart to `shruti-mcp` (which writes
...
the origin host inside the `shruti` docker network (reaches
`postgres:5432`) and is published **only on the host's Tailscale IP**
(`SHRUTI_TS_IP`) — the curator's machine reaches it over the tailnet; the
