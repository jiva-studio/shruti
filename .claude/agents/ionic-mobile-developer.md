...
| Alias | Path | Purpose |
|-------|------|---------|
| `@shruti/*` | `./shruti/*` | Composition root + driving adapters |
...
| `audio/capacitor/` | `IAudioPlayer` wrapping `@shruti/plugin-audio-player` plugin |
...
### Composition Root (`shruti/`)

Outermost layer — wires everything together. May import all layers.

```
shruti/
├── shruti.ts                 # Singleton — wires all adapters, lazy repositories()
├── main.ts                      # Entry point
...
`shruti/shruti.ts` is the only file that knows every concrete adapter. It exposes a `repositories()` method that lazily builds the SQL repository bundle on first call (after databases are opened by the Welcome view):
...
- **Content DB** (`shruti.{version}.db`) — prebuilt SQLite shipped inside APK/IPA and updated from CDN. Read-only at runtime.
- **User DB** (`user.db`) — created on device, migrations in `shruti/services/migrations/user/`. Notes, playlist, media state.
