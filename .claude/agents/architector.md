...
│       │   ├── shruti.ts                 # Singleton — wires all adapters
│       │   ├── main.ts                      # Entry point
...
- **Composition Root:** `shruti/shruti.ts` — only file knowing all concrete adapters, exposes lazy `repositories()`.
