...
CHAT_URL="http://localhost:${SHRUTI_CHAT_PORT:-11080}"
MODE="${1:-all}"
...
ensure_stack() {
  ...
  # Find a shruti checkout that has the stack config (.env.dev). Prefer a
  # sibling main checkout; fall back to this worktree's repo root.
  local repo=""
  for cand in "$E2E_DIR/../../../../shruti" "$E2E_DIR/../../.."; do
    [[ -f "$cand/infra/app/.env.dev" ]] && { repo="$(cd "$cand" && pwd)"; break; }
  done
  if [[ -z "$repo" ]]; then
    echo "!! no infra/app/.env.dev found — run 'make stack-setup' in your shruti checkout first" >&2
    exit 1
  fi
