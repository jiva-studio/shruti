...
test(qase(65, caseTitle(65)), { tag: ["@offline", "@library"] }, async ({ page }) => {
  ...
  await step(page, 65, 1, async () => {
    ...
    // sheet rather than the paywall. Rendering the PDF itself is server-side (a
    // @live concern) — here we assert the export entry points exist. Labels come
    // from i18n locales/en/search.ts → search.share.*: "Transcript (PDF)",
    // "Transcript (text)", "Audio". Audio is enabled because the seeded track
    // variant has the silent-MP3 stub. Match by rendered i18n label, not index.
    const sheet = page.locator("ion-action-sheet")
    await expect(sheet).toBeVisible({ timeout: 15_000 })
    await expect(sheet.getByRole("button", { name: /transcript \(pdf\)/i })).toBeVisible()
