...
- **Network interception** — `**/public/config.json` and the catalog
  `**/public/db/lectorium.*.db` are fulfilled from `fixtures/` so the app comes
...
a `LECTORIUM_TS_IP` (only the search-mcp service; a dummy is fine). For grounded
...
- `LECTORIUM_STORAGE_KEY`, `LECTORIUM_YC_ACCESS_KEY_ID`,
  `LECTORIUM_YC_SECRET_ACCESS_KEY` — `storage-sync` only. Dummies are fine; the
...
- the JWT keypair must be **readable by the auth container's user**. The keys
  ...
  `LECTORIUM_JWT_KEYS_DIR` at a copy the container can read.
