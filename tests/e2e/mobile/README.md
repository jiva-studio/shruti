...
- **The build decides whether the seam exists.** `__E2E_BUILD__` is compiled in
  from `SHRUTI_E2E_BUILD=1` (`modules/apps/mobile/vite.config.ts`). Only a
...
`playwright.config.ts` passes `SHRUTI_E2E_BUILD=1` to the dev server, so
...
Whichever you take, it has to carry shruti-mcp's `008_fold_fts_marks` first.
