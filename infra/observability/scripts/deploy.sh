...
#   4. rsync the compose tree + scripts to /opt/lectorium-observability.
...
docker run --rm -v lectorium-observability_loki-data:/loki alpine \
...
#     so paths like `compose/.env` and `docker exec lectorium-observability-*-1`
