...
#
# Container naming: docker compose v2 with `name: shruti-observability`
# produces `shruti-observability-<service>-1`. The Langfuse tables live
# in the `default` ClickHouse database (despite CLICKHOUSE_DB=langfuse in
# compose, Langfuse v3 migrations target `default`).
set -euo pipefail

docker exec -i shruti-observability-clickhouse-1 \
