...
entirely in `/opt/shruti/.env` on the host (`SHRUTI_REGION_ID`,
...
2. Place both old and new on the VPS:
   ```bash
   scp <new-private> root@<ip>:/opt/shruti/jwt/private.pem.v2
   scp <new-public>  root@<ip>:/opt/shruti/jwt/public.pem.v2
   ```
3. Update auth to dual-sign / dual-verify — code change, separate task.
4. Cut new tokens by bumping `SHRUTI_JWT_KID=v2` in `.env` and
   redeploying auth.
