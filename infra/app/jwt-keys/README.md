...
single-key shape from `/opt/shruti/jwt/{private,public}.pem`. The
...
>
> 1. Place the priv key at `/opt/shruti/jwt/russia-v1.priv.pem` on
...
1. **On the rotating region's VPS** generate the new keypair:
   ...
   ```
   Writes `/opt/shruti/jwt/global-v2.priv.pem` (mode 0400) and
   `/opt/shruti/jwt/global-v2.pub.pem` (mode 0644).

2. **Commit the new public part** to this repo:
   ```bash
   cp /opt/shruti/jwt/global-v2.pub.pem $REPO/infra/app/jwt-keys/
...
5. **After 90 days**, retire the previous key:
   ...
   sudo rm /opt/shruti/jwt/public.pem /opt/shruti/jwt/private.pem
   ```
