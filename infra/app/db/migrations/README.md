# lectorium DB migrations

Single source of truth for the schema of the `lectorium` postgres database.
...
```bash
# On the VPS:
docker compose -p lectorium run --rm migrator force <N>   # after manual SQL fixup
docker compose -p lectorium up -d migrator                 # retry apply
```
