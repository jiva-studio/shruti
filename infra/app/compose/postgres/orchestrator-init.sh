...
SELECT 'CREATE USER shruti_exporter'
 WHERE NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'shruti_exporter')\gexec
...
ALTER USER shruti_exporter WITH PASSWORD :'pw';
...
echo "orchestrator-init: shruti_exporter ready"
