#!/usr/bin/env bash
# One-shot: nuke the legacy /opt/shruti-chat stack + its volumes.
#
...
#     prefixes (shruti-chat_*, chat_*, compose_*). The new stack uses
#     `name: shruti` so all its volumes are prefixed `shruti_*` —
#     this script intentionally never touches those.
#   - /opt/shruti-chat removal is rm -rf; that directory is the
...
LEGACY_VOLUMES=(
  "shruti-chat_pgdata"
  "shruti-chat_chatdata"
  "shruti-chat_caddydata"
  "shruti-chat_caddyconfig"
  "chat_pgdata"
  "chat_chatdata"
  "chat_caddydata"
  "chat_caddyconfig"
...
if ! $have_dir && ! $have_vol; then
  echo "✓ no legacy /opt/shruti-chat or chat_*/compose_*/shruti-chat_* volumes — nothing to do."
  exit 0
fi
