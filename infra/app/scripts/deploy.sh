#!/usr/bin/env bash
# Push the lectorium stack onto a VPS. Code-only — secrets stay on the host.
#
# Pre-deploy (one-time per VPS, by hand):
#   1. mkdir -p /opt/lectorium/jwt
#   2. scp /opt/lectorium/.env (LECTORIUM_POSTGRES_PASSWORD etc. — see
...
# ── 3.5. Mirror cross-region JWT pubkeys into /opt/lectorium/jwt/. ───
...
# /secrets — which mounts /opt/lectorium/jwt — so any `*.pub.pem` we
