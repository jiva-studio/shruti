...
|---|---|---|---|
| `shruti-promtail` | `grafana/promtail:3.3.0` | `${PROD_EU_TS_IP}:9080` (self-metrics) | Tails Docker JSON log files, pushes to Loki on obs. |
| `shruti-node-exporter` | `prom/node-exporter:v1.8.2` | `${PROD_EU_TS_IP}:9100` | Host CPU / RAM / disk / net (network_mode: host). |
...
ssh prod-eu '
  docker exec -i shruti-postgres-1 psql -U shruti -d shruti <<SQL
    CREATE EXTENSION IF NOT EXISTS pg_stat_statements;
    DO \$\$ BEGIN
  ...
  SQL
  cd /opt/shruti && docker compose restart postgres
'
```
...
```bash
ssh prod-eu 'docker exec -i shruti-postgres-1 psql -U shruti -d shruti -c "\du pg_exporter"'
```

Note: the container name above (`shruti-postgres-1`) assumes the
compose project name `shruti` (it is — see
`infra/compose/docker-compose.yml`, top-level `name: shruti`).
...
| Target | Module | Notes |
|---|---|---|
| `http://chat:8080/healthz` | `http_2xx` | chat-stack network DNS — works because blackbox is on `shruti_shruti`. |
...
- `shruti_share_video_queue_depth` — `count(*) FROM tasks WHERE state='pending'`
- `shruti_share_video_oldest_pending_seconds` — oldest pending task age
- `shruti_share_video_tasks_by_state{state=...}` — distribution
...
```bash
ssh prod-eu 'cd /opt/shruti-obs-agent/compose && docker compose --env-file ../.env down'
```
