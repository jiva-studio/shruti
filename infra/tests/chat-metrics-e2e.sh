...
NET=lectorium-chat-metrics-e2e
STUB=lcm-e2e-chat
PROXY=lcm-e2e-proxy
PROM=lcm-e2e-prom
PROXY_PORT=19119
...
docker build -q -t lectorium-chat-metrics-stub:test "$STUB_DIR" >/dev/null
...
docker run -d --name "$STUB" --network "$NET" --network-alias chat \
  lectorium-chat-metrics-stub:test >/dev/null
...
if grep -qE '^lectorium_chat_turns_in_flight ' "$WORKDIR/body.txt"; then
  pass "body carries a real sample: $(grep -E '^lectorium_chat_turns_in_flight ' "$WORKDIR/body.txt" | head -1)"
else
  bad "body has no lectorium_chat_turns_in_flight sample"
fi
...
value=$(query 'lectorium_chat_turns_in_flight{job="chat-prod-eu",host="prod-eu",service="chat"}' 40)
if [ "$value" = "3" ]; then
  pass "Prometheus scraped lectorium_chat_turns_in_flight = 3 with job/host/service labels"
else
