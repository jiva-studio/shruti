...
The mobile app lives in `modules/apps/mobile/`. Shared TypeScript libraries live in `modules/libs/`. In-house Capacitor plugins (native Android/iOS/web plugin packages) live in `modules/plugins/` and are published under the `@shruti/plugin-*` npm scope. The content-db-builder tool lives in `modules/tools/`. Nothing else.
