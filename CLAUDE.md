...
Lectorium is a mobile-only app that serves lectures (audio + transcripts) from a public S3 bucket. No backend, no authentication, no telemetry — just the Ionic/Vue app and static files on S3.
...
The mobile app lives in `modules/apps/mobile/`. Shared TypeScript libraries live in `modules/libs/`. In-house Capacitor plugins (native Android/iOS/web plugin packages) live in `modules/plugins/` and are published under the `@lectorium/plugin-*` npm scope. The content-db-builder tool lives in `modules/tools/`. Nothing else.
