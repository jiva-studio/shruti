...
```
main.ts
  └─ initLectorium(config)                       [modules/apps/mobile/lectorium/lectorium.ts]
...
- `modules/apps/mobile/lectorium/main.ts` — creates Vue + Ionic, waits for router,
  calls `initLectorium(config)` before mounting.
- `modules/apps/mobile/lectorium/lectorium.ts` — `initLectorium(config)` assembles a
...
- **Active server** — stored in `lectorium.activeServer`; preferred id
...
3. Keep only `lectorium.*.db` file names not present in the in-memory
...
1. Open user DB at a fixed path (`user.db` on native, `lectorium/databases/user.db`
...
`modules/apps/mobile/lectorium/services/migrations/user/` (individual
...
3. Run `npm run upload` to push the DB and any changed transcripts to
   every CDN bucket as `/public/db/lectorium.{new_version}.db`, and update
