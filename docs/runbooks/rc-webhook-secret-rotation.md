...
- The auth service has been redeployed since `feat/rc-bearer-hardening`
  ...
  Confirm with `curl https://auth.lectorium.app/metrics | grep rc_webhook_auth_total`;
...
```bash
LECTORIUM_RC_WEBHOOK_SECRET_PRIMARY=<existing-value>          # unchanged
LECTORIUM_RC_WEBHOOK_SECRET_SECONDARY=<new-value-from-step-1>
```
...
```bash
LECTORIUM_RC_WEBHOOK_SECRET_PRIMARY=<value-that-was-secondary>
LECTORIUM_RC_WEBHOOK_SECRET_SECONDARY=
```
...
- Confirm the legacy `LECTORIUM_RC_WEBHOOK_SECRET` env var is empty in
...
- If the rotation was triggered by a leak, also rotate the RC REST API
  key (`LECTORIUM_RC_REST_API_KEY`) — same dashboard, different section.
...
You cleared both slots and the legacy `LECTORIUM_RC_WEBHOOK_SECRET` env
