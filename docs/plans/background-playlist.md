...
### JS / app layer
- **`usePlayerStore`** (`modules/apps/mobile/lectorium/stores/usePlayerStore.ts`) — singleton player
...
- `modules/apps/mobile/lectorium/stores/usePlayerStore.ts` — build queue in `openTrack`; drain on init/resume
- composition root (app bootstrap) — run the journal drain early, before playlist badge compute
- `modules/apps/mobile/lectorium/stores/player/usePlayerSession.ts` — foreground completion → advance
- `modules/apps/mobile/lectorium/stores/usePlaylistStore.ts` — "next entry after itemId" selector
...
- `modules/apps/mobile/lectorium/views/Settings/SettingsView.controller.ts` — `useConfig` key
- `modules/apps/mobile/ui/features/subscription/featureKeys.ts` — new slide(s)
- `modules/apps/mobile/lectorium/i18n/locales/{en,ru}/settings.ts` — copy
