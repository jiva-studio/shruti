...
1. **Remove from the catalog** — a `library.depublish <track_id>` admin op that deletes the track's rows from the corpus DB (the inverse of `track.commit`) and re-runs `catalog.publish`, so the next `shruti.{version}.db` no longer contains it. Clients drop it on their next content-DB refresh.
