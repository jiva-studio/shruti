...
1. **Merge → CI green.** Confirm GHCR has `ghcr.io/jiva-studio/shruti-orchestrator:latest` and rebuilt `-chat`, `-profile`, `-migrator`.
...
- Pin the previous image tag in `/opt/shruti/.env` (e.g. `SHRUTI_ORCHESTRATOR_TAG=main-<sha>`) and re-run `deploy.sh`.
