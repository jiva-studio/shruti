...
The views (routes defined in `modules/apps/mobile/shruti/router/index.ts`; the tab bar in `views/TabsLayout.vue` mounts Home, Search, Chat, Notes and Settings):
...
A single composable per surface converts domain inputs into UI types — `buildTrackRow` (`shruti/composables/buildTrackRow.ts`) produces `UiTrackRow`, and `buildTranscriptViewData` (`shruti/composables/buildTranscriptViewData.ts`) builds the transcript view model. Adding a new field is: domain → mirror → builder → template.
...
Is it bound to a route / does it own state?
  └─ shruti/views/<View>/  (with a sibling .controller.ts)
```
