...
graph LR
    ...
    subgraph app["App"]
        ...
        ACTIVE[("shruti.activeServer<br/>Ref&lt;CdnServer&gt; (in-memory)")]
...
| `preferredServerId` | `shruti.activeServer.value.id` (seeded from `IPreferences['preferredServerId']`) | Reorders the list to try the user's last-known-good region first |
...
stateDiagram-v2
    ...
    note right of Active
        shruti.activeServer (Ref<CdnServer>).
...
| `public/db/shruti.{ver}.db` | Local filesystem under `shruti/databases/` | On scheme rejection / corrupt-file detection — `databaseFetcher.delete(path)`; otherwise kept until next launch picks a newer file |
