...
- `SUPPORTED_LOCALES` (in `modules/apps/mobile/shruti/i18n/index.ts`) + one i18n strings file per feature per locale (`shruti/i18n/locales/<locale>/*.ts`); fallback to `en` is generic. Today the set is `en, ru, uk, sr-Latn, sr-Cyrl, es, pt, it, de, fr, pl, hu, hi, bn`.
