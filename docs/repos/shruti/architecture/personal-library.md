...
graph LR
    ...
    subgraph reuse[extracted from shruti-mcp]
        SHARED
    end
    MCP[shruti-mcp] -.-> SHARED
...
| Approve public promotion | `shruti-mcp` (admin tools) | It owns the corpus DB and dictionaries; reads `pending.db` |
...
- **`track.published`** — `publish-service` emits `{track_id, owner_id}` once a track is promoted into the shared corpus. Consumers: **`profile`** flips its `library_items.origin` to `published` (via the server-authored `MarkPublished` path), and **`chat`** runs the RAG *graft* (relabel `user_track` → `track_transcript`, drop the per-user `owned` rows) so the track becomes corpus-wide.
- **`library.unlinked`** — `profile` emits `{user_id, track_id}` when a user removes an item (one of the few places `profile` is a producer, not just a consumer); `chat` consumes it to drop the row from its `owned` projection so the track leaves that user's RAG scope.
...
- **Real dedup** — if user B adds a video user A already processed, both `library_items` point at the same blob (same hash); the second ingest is a no-op.
...
**`library_items` is the source of truth for ownership.** It is per-user membership: one row per `(user_id, track_id)`. Several users adding the **same** lecture produce several rows sharing one `track_id` — each with their own status plus their own listening/notes — over a **single** shared blob and a single shared set of RAG chunks (both keyed by `track_id`). The second user's ingest dedups (content exists) and only emits `track.linked`; nothing is re-fetched, re-transcribed or re-indexed. Removing one user's row leaves the shared content intact for the others (refcount GC when the last owner leaves).
...
**Two mechanisms — don't mix them up.** Self-run embedded migrations (the `profile` pattern) are only for services with their **own** Postgres. Services on the **shared `shruti` DB** (`chat`, auth, billing, …) migrate through the central golang-migrate `migrator` container against `infra/app/db/migrations/` — `chat` has **no** migration runner of its own (an earlier draft wrongly said it self-migrates).
...
- **In-flight dedup.** Two users submitting the same video simultaneously both miss the "already done" check → double fetch + a write race on `content/<hash>`. A **claim-lock on the content hash** (advisory lock / unique `content_jobs.hash`) makes the second attach to the first instead of duplicating work.
- **Failure path.** fetch-ok / transcribe-fail must set `status=failed` with a user-visible reason and a retry affordance. Poison jobs (age-restricted, removed, geo-locked) must hit a **max-attempts dead-letter** so Redis redelivery does not loop forever. Orphaned blobs from failed jobs are cleaned up.
- **Input limits.** Enforce max duration / max size at submit to cap Deepgram cost (a 10-hour stream or a movie should be rejected).
- **Transcript adapter.** A **Deepgram → internal `raw.json`** adapter is required so the shared review usecase and the client transcript renderer see the format they expect; include language detection. Quality caveat: Deepgram handles **Sanskrit / IAST verses** poorly, so such lectures transcribe below corpus quality.
