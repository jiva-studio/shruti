...
- **`track.events`** — the **orchestrator** (not the worker) emits track lifecycle events (`track.queued` / `track.processing` / `track.ready` / `track.failed`) with `{track_id, owner_id, keys, metadata, transcript_ref}`, by translating the worker's `ingest.result` phase onto a job-state transition. Event ids are derived from the **`job_id`** — stable across both streams and across redelivery, so downstream consumers stay idempotent. This is a **fan-out**: independent consumer groups subscribe —
  ...
  - **`chat`** indexes the transcript into its private RAG index on `track.ready`, and records `(owner_id, track_id)` in its local `owned` projection on `track.ready`,
...
- **Failure path.** fetch-ok / transcribe-fail must set `status=failed` with a user-visible reason and a retry affordance. Poison jobs (age-restricted, removed, geo-locked) are non-retriable and hit a **max-attempts dead-letter** so Redis redelivery does not loop forever. Orphaned blobs from failed jobs are cleaned up.
