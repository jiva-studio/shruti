...
Account deletion erases **all** of a user's synced data, chat included, and **`profile` erases it itself** — it is simply *told* to, over HTTP. It connects to no database but its own and is **not** an outbox consumer; the service knows nothing about the shared `shruti` schema.
...
| Scheduler (startup / interval / resume / flush / sign-in) — triggers use-cases only | `shruti/` | `composables/useSyncEngine.ts`, `services/syncEvents.ts` |
| Post-pull store refresh (event bus) | `shruti/` | Pinia stores |
...
`profile` is origin-only, JWT-bearing, and operationally identical to `auth`/`chat` — except it owns its **own Postgres** and carries its **own embedded migrations** (a deliberate departure from the shared `shruti` DB + central migrator, to isolate small frequent user-data writes from the chat/RAG load).
