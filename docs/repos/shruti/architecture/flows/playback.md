...
sequenceDiagram
    ...
    participant Plugin as @shruti/plugin-audio-player
...
The pure logic in `pickVariantWithAudio` ([`playTrack.ts`](https://github.com/akdasa-studios/shruti/blob/main/modules/apps/mobile/usecases/playback/playTrack.ts)):
...
`IAudioPlayer` (a tech port in `@ports/app`, file `modules/apps/mobile/ports/app/audioPlayer.ts`) wraps the platform-specific player. It is implemented by a single cross-platform adapter, `useCapacitorAudioPlayer` (`modules/apps/mobile/infra/audio/capacitor/useCapacitorAudioPlayer.ts`), over the `@shruti/plugin-audio-player` Capacitor plugin. The plugin owns the native engine on Android/iOS and falls back to `HTMLAudioElement` on web — there is no separate web sibling.
...
`usePlayerStore` ([`usePlayerStore.ts`](https://github.com/akdasa-studios/shruti/blob/main/modules/apps/mobile/shruti/stores/usePlayerStore.ts)) is an app-singleton holding the reactive shape (`trackId`, `title`, `authorName`, `language`, `playing`, `positionMs`, `durationMs`, `itemId`, `mixPosition`, `playbackSpeed`) and orchestrating `openTrack / togglePause / pause / seek / skipBack / skipForward / playNext / playPrevious / stop`.
