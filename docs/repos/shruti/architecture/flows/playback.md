...
The pure logic in `pickVariantWithAudio` ([`playTrack.ts`](https://github.com/jiva-studio/shruti/blob/main/modules/apps/mobile/usecases/playback/playTrack.ts)):
...
`usePlayerStore` ([`usePlayerStore.ts`](https://github.com/jiva-studio/shruti/blob/main/modules/apps/mobile/shruti/stores/usePlayerStore.ts)) is an app-singleton holding the reactive shape (`trackId`, `title`, `authorName`, `language`, `playing`, `positionMs`, `durationMs`, `itemId`, `mixPosition`, `playbackSpeed`) and orchestrating `openTrack / togglePause / pause / seek / skipBack / skipForward / playNext / playPrevious / stop`.
