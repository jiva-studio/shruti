...
There is no separate note-editor dialog or seconds-based range picker: a note IS the selected transcript span. A second entry point — saving a chat citation chip — reuses the same `createNote()` use case through [`saveCitationAsNote`](https://github.com/jiva-studio/shruti/blob/main/modules/apps/mobile/usecases/chat/saveCitationAsNote.ts).
