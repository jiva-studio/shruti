...
- Each `deploy.sh` is idempotent: render templates → rsync the unit to `/opt/lectorium*/` → `docker compose pull && up -d` → health-check.
- Secrets stay on the host (`/opt/lectorium/.env`, `/opt/lectorium/jwt/`, per-unit `secrets/`) and are **never** copied by the scripts; `ensure_secrets` generates-once-and-reuses.
...
- The exporter Postgres role is `lectorium_exporter` (granted `pg_monitor` plus explicit `SELECT` on `public.tasks` and `app.outbox`); seeded by `infra/app/compose/postgres/init.sql` on a fresh volume, applied manually on existing volumes.
...
```mermaid
graph LR
    subgraph builder["lectorium-mcp<br/>(Go CLI / MCP)"]
        BUILD["catalog.publish · library.publish"]
    end

    subgraph s3["akds-lectorium S3 bucket"]
        PUB[("public/<br/>config.json · db/ · tracks/")]
        ART[("private/ · artifacts/<br/>internal-only")]
    end

    subgraph cdn["CDN mirrors"]
        AWS["AWS us-east-1<br/>akds-lectorium.s3.us-east-1.amazonaws.com"]
        YA["Yandex Cloud (RU)<br/>akds-lectorium.storage.yandexcloud.net"]
    end
...
The prebuilt content DB and `public/config.json` are published from a developer machine via the lectorium-mcp pipeline: `catalog.publish` uploads `public/db/lectorium.{ver}.db` and flips `public/config.json` (and `library.publish` merges the library entry into the same manifest). The Yandex mirror is kept in sync from AWS by the `storage-sync.yml` workflow (rclone). The hosted region services also read and write the bucket: `share-audio` cuts `public/shares/audio/*.mp3` on demand, `share-video` renders into `public/share/video/` (scratch + backgrounds under `private/share/video/`), and `share-transcript` renders printable PDFs to `public/tracks/{id}/exports/{lang}.pdf` (the caller supplies the lecture outline in the request body — share-transcript does not generate or cache outlines; they are produced offline by lectorium-mcp and stored in the catalog DB, where `chat` reads them and forwards them to the client / share-transcript). Backup cron (`infra/app/scripts/backup.sh`) writes `pg_dump` archives to `private/backups/postgres/`.

## Read-paths and write-paths

| Path | Reader | Writer |
|---|---|---|
| `public/config.json` | App on every cold start + background refresh | lectorium-mcp `catalog.publish` / `library.publish` |
| `public/db/lectorium.{ver}.db` | App when no compatible DB is cached locally | lectorium-mcp `catalog.publish` |
| `public/tracks/{id}/audio/original.mp3` | Audio player + offline downloader | track pipeline (lectorium-mcp), pushed to S3 |
| `public/tracks/{id}/transcripts/{lang}.json` | `ITranscriptRepository` (HTTP) on demand | track pipeline (lectorium-mcp), pushed to S3 |
...
The publish pipeline (`catalog.publish` / `library.publish`) lives in the lectorium-mcp tool (`modules/tools/lectorium-mcp/`).
...
- **Write** (lectorium-mcp publish): uploads to every configured S3 target — AWS always, Yandex when the `S3_YANDEX_*` env vars are set (leave unset to skip the mirror). The Yandex mirror is also kept in sync externally via rclone (`.github/workflows/storage-sync.yml`), so AWS-only uploads are mirrored asynchronously.
- **Region services**: each workload uses a scoped AWS IAM key — `lectorium-share-audio`, `lectorium-share-video`, `lectorium-chat`, `lectorium-backup` (least-privilege policies enumerated in `infra/app/README.md`). The shared `/opt/lectorium/.env` key is the union of these policies.
- **ghcr pull**: the region VPS holds a `read:packages` PAT under `/opt/lectorium/config/config.json`; `deploy.sh` reads it via `DOCKER_CONFIG` and the prod overlay bind-mounts it for Watchtower.
