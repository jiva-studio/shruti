...
Day-to-day Lectorium development is the Ionic + Vue mobile app talking to
...
The repo root has a `Makefile` wrapping every common command. Run `make help` from `source/lectorium/` for the full list; below is the daily subset.
...
| Services | `make transcriber-up` / `-down` / `-status` / `-logs`, `make lectorium-mcp-up` / `-down` / `-status` / `-logs` |
...
The wrapper (`modules/db-sync.sh`) is thin: it exports Lectorium's app
...
Stale `lectorium.*.db` files in each target are removed so lexicographic
pickup stays correct.

The DB itself is produced and published by the `lectorium-mcp`
...
(`lectorium`), the `origin` compose profile, the file set and the
`infra/app/.env.dev` env file:

```make
STACK_COMPOSE = COMPOSE_PROFILES=origin docker compose -p lectorium \
...
`infra/app/.env.dev` with `LECTORIUM_`-prefixed vars, idempotent) and
...
(each overridable via the matching `LECTORIUM_*_PORT` var). `migrator`,
