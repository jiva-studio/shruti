# lectorium-mcp daemon

`lectorium-mcp` is a long-running HTTP/SSE MCP daemon that owns the
...
All commands run from `modules/tools/lectorium-mcp/`:

| Command | What it does |
|---|---|
| `make build` | Compile `./bin/lectorium-mcp` |
| `make up` | Start in background. Pid → `/tmp/lectorium-mcp.pid`, log → `/tmp/lectorium-mcp.log`. Idempotent (refuses if already running). |
...
`$(CURDIR)` — the `lectorium-mcp/` module root where the Makefile lives)
before exec, so the binary picks up `./lectorium-mcp.yaml` + `./.env` from
...
-config <path>                 # YAML config (default: ./lectorium-mcp.yaml)
```

## Connecting Claude Code

Edit `.mcp.json` in the project where you want lectorium-mcp tools:
...
(`artifacts/catalog/current.db` → `public/db/lectorium.{ver}.db`) and an
...
**Heartbeat in log** — one line per step (`/tmp/lectorium-mcp.log`):
...
`transcribe.providers.transcriber-service.endpoint` in `lectorium-mcp.yaml`
...
/tmp/lectorium-mcp.log`.

### Ghost lectorium-mcp processes

A crashed-but-not-reaped session can leave a stray daemon holding the
port. `make down` already sweeps any `pgrep -x lectorium-mcp` match, but
to clean up by hand:

```sh
pgrep -fa lectorium-mcp     # what's running
pkill -x  lectorium-mcp     # kill by exact comm-name (won't match shell-strings)
...
```
modules/tools/lectorium-mcp/lectorium-mcp.example.yaml   # template, DON'T edit
agent/lectorium-mcp.yaml                                  # your live config
...
- README: `modules/tools/lectorium-mcp/README.md`
- Entry point + wiring: `modules/tools/lectorium-mcp/cmd/lectorium-mcp/main.go`
- Tool registration: `modules/tools/lectorium-mcp/internal/mcp/tools/` (`RegisterAll` in `pipeline.go`)
- Envelope: `modules/tools/lectorium-mcp/internal/mcp/envelope/envelope.go`
- Source: `modules/tools/lectorium-mcp/internal/`
