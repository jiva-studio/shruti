...
| Group | Files | Purpose |
|---|---|---|
| Identity / scalars | [`core.ts`](https://github.com/akdasa-studios/lectorium/blob/main/modules/libs/domain/core.ts) | `TrackId`, `AuthorId`, `NoteId`, `PlaylistItemId`, `MediaItemId`, `ChatSessionId`, `ChatMessageId`, `LanguageCode`, `IsoDate`, `UnixMs`, … |
| Result helper | [`@kit/core`](https://github.com/akdasa-studios/lectorium/blob/main/modules/kit/src/core) (shared kernel) | `Result<T, E>` discriminated union (+ `ok` / `err` constructors) for recoverable errors — one shared type across domain, application, and infra |
