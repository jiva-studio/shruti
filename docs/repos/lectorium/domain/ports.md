...
Ports are TypeScript interfaces that the domain depends on but does not implement. Use cases consume them; concrete adapters under `modules/apps/mobile/infra/repositories/` provide them; the composition root in [`lectorium.ts`](https://github.com/jiva-studio/lectorium/blob/main/modules/apps/mobile/lectorium/lectorium.ts) wires the two together. There are **16 domain ports** under [`modules/libs/domain/ports/`](https://github.com/jiva-studio/lectorium/tree/main/modules/libs/domain/ports) — 15 repositories plus `IUnitOfWork`. The barrel [`ports/index.ts`](https://github.com/jiva-studio/lectorium/blob/main/modules/libs/domain/ports/index.ts) re-exports most of them; `ITopicRepository` is imported directly from its module by the discovery use cases. A separate set of **technical ports** (see the layer doc) covers non-domain concerns like the audio player or the file system.
...
Source: [`unitOfWork.ts`](https://github.com/jiva-studio/lectorium/blob/main/modules/libs/domain/ports/unitOfWork.ts) · Implementation: `modules/apps/mobile/infra/repositories/sql/unitOfWork.sql.ts`
