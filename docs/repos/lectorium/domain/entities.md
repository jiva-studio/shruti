...
### `Track` — [`track.ts`](https://github.com/akdasa-studios/lectorium/blob/main/modules/libs/domain/track.ts)
...
### `TrackVariant` — [`trackVariant.ts`](https://github.com/akdasa-studios/lectorium/blob/main/modules/libs/domain/trackVariant.ts)
...
`TrackAudio.path` and `TrackTranscriptRef.path` are **full bucket keys** including the `public/` prefix (e.g. `public/tracks/abc123/audio/original.mp3`). The client never concatenates prefixes — see [`useStoragePublicUrl.ts`](https://github.com/akdasa-studios/lectorium/blob/main/modules/apps/mobile/infra/storagePublicUrl/useStoragePublicUrl.ts) and [Storage layout](../infra/s3-layout.md).

`TrackAudio.duration` is in **milliseconds**.

### `Reference` — [`reference.ts`](https://github.com/akdasa-studios/lectorium/blob/main/modules/libs/domain/reference.ts)
...
These entities live in the per-device user DB built by [`runMigrations.ts`](https://github.com/akdasa-studios/lectorium/blob/main/modules/apps/mobile/infra/persistence/migrations/user/runMigrations.ts). They never leave the device — there is no sync.

### `Note` — [`note.ts`](https://github.com/akdasa-studios/lectorium/blob/main/modules/libs/domain/note.ts)
...
### `PlaylistItem` — [`playlistItem.ts`](https://github.com/akdasa-studios/lectorium/blob/main/modules/libs/domain/playlistItem.ts)
...
### `ListeningSession` — [`listeningSession.ts`](https://github.com/akdasa-studios/lectorium/blob/main/modules/libs/domain/listeningSession.ts)
...
### `MediaItem` — [`mediaItem.ts`](https://github.com/akdasa-studios/lectorium/blob/main/modules/libs/domain/mediaItem.ts)
...
Stale `downloading` rows on app start are flipped to `failed` by [`IMediaItemRepository.failStaleDownloads()`](https://github.com/akdasa-studios/lectorium/blob/main/modules/libs/domain/ports/mediaItemRepository.ts) so a force-close mid-download doesn't permanently lock the row with `already-in-progress`. See [`downloadMedia.ts`](https://github.com/akdasa-studios/lectorium/blob/main/modules/apps/mobile/usecases/downloads/downloadMedia.ts) and [`removeDownloadedMedia.ts`](https://github.com/akdasa-studios/lectorium/blob/main/modules/apps/mobile/usecases/downloads/removeDownloadedMedia.ts) for the full transition rules.

### `ChatSession` — [`chatSession.ts`](https://github.com/akdasa-studios/lectorium/blob/main/modules/libs/domain/chatSession.ts)
...
### `ChatMessage` — [`chatMessage.ts`](https://github.com/akdasa-studios/lectorium/blob/main/modules/libs/domain/chatMessage.ts)
...
`Transcript` — [`transcript.ts`](https://github.com/akdasa-studios/lectorium/blob/main/modules/libs/domain/transcript.ts) — is downloaded as JSON from S3 on demand and **never written to SQLite**. It is a sequence of typed blocks:
