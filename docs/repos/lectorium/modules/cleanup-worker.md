...
- **Prometheus gauge** `lectorium_outbox_pending_seconds{event_type}` = max age of unprocessed rows per type, refreshed every 30 s and `Reset()` each poll so a drained type stops reporting. A Grafana alert (`outbox_dead_letter`) fires off this gauge → Telegram when a type stays stuck. Exposed on `/metrics` alongside `/healthz`.
