...
All three return the standard envelope. The exposed tool names are `search`, `search_get`, `search_window` (the client sees them namespaced as `mcp__lectorium-search__*`).
...
Transport is **streamable HTTP MCP** at `/mcp` (stateless), plus a legacy SSE pair `/sse` + `/message`, and `/healthz`. The binary listens on `0.0.0.0:8086` inside the container, but compose publishes the port **only on the Tailscale IP** (`${LECTORIUM_TS_IP}:8086:8086`). Clients reach it over the tailnet:
...
| Var | Default | Notes |
|---|---|---|
| `DATABASE_URL` | required | pgvector DSN (the prod `lectorium` DB). |
...
- **Read-only** — it never writes; all curation writes go through lectorium-mcp's `library.attribution.*` against `catalog/current.db`.
- **Tailnet-only** — not reachable from the public internet; you need to be on the tailnet (`lectorium-prod-eu`).
