...
| `@usecases`, `@ports/*`, `@infra/*`, `@lectorium/*` | Same inward rule the app-local `ui/**` layer obeys |
...
3. Give each host a thin container (`lectorium/views/Chat/components/*Container.vue` on mobile, `src/components/vue/Web*.vue` on web) that resolves the data.
