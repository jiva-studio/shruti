...
> `lectorium/services/startup.ts`. See [Onboarding](onboarding.md).
...
```
modules/apps/mobile/lectorium/main.ts
  └─ installConsoleCapture()                          [services/logger]
  └─ initLectorium(seed)                              [lectorium/lectorium.ts]
  ...
  └─ Welcome view                                     [views/Welcome/WelcomeView.controller.ts]
      ...
      Phase migrate  — bootstrapUserDatabaseFromApp       [lectorium/services/bootstrap.ts]
...
```mermaid
graph TD
  M["main.ts: initLectorium(seed) + hydrateRegions"] --> R["router.isReady → mount App.vue"]
...
- `modules/apps/mobile/lectorium/main.ts` — first installs `installConsoleCapture()`
  (Settings → Debug → "View logs"), then creates Vue + Pinia + Ionic + i18n +
  router. Crucially it calls `initLectorium(seed)` **before** `router` is
  installed, because the first navigation runs `beforeEach` synchronously and
  that guard calls `useLectorium()` / `isLectoriumInitialized()`. It wires Sentry
  via `initMonitoring(app)` before mount, then awaits `hydrateRegions(preferences)`
  and `router.isReady()` before `app.mount("#app")`.
- `modules/apps/mobile/lectorium/lectorium.ts` — `initLectorium(seed)` builds the
  composition-root singleton (`Lectorium`) holding every concrete adapter.
  ...
  - FilesStorage: `useCapacitorRemoteFilesStorage({ cacheDir: "lectorium" })`
    or `useWebRemoteFilesStorage({ cacheName: "lectorium" })`.
  - Plus audio, mediaDownloader, notifications, share (audio/video/transcript),
    ...
    transfer — all listed in the `InitLectoriumSeed` interface or constructed
    locally inside `initLectorium`.
  - `storagePublicUrl` is built **inside** `initLectorium` via
...
- On native, `main.ts` overrides `config.database.userLocalPath` to `"user.db"`
  ...
  the `DEFAULT_APP_CONFIG` value `"lectorium/databases/user.db"`.
...
published `public/config.json` (managed via lectorium-mcp
`catalog.config.regions.*`) and is hydrated at runtime by
`lectorium/services/regionsRegistry.ts`:
...
- **Active server** — `lectorium.activeServer` (a `Ref<CdnServer>`). A `watch`
  inside `initLectorium` mirrors the id into the registry (`setActiveRegionId`,
  so asset URLs follow the live region) and persists it under
  `PREFERRED_SERVER_KEY` (`"preferredServerId"`, see
  `lectorium/services/preferredServer.ts`) whenever it changes, so the same
...
`.db` under `assets/databases/` into `getFilesDir()/lectorium/databases/`, so even
...
`bootstrapUserDatabaseFromApp(lectorium)` (`lectorium/services/bootstrap.ts`):

1. `openUserDatabase(userLocalPath)` opens the user DB at the configured path
   (`user.db` on native, `lectorium/databases/user.db` on web).
...
| Content DB    | `/public/db/lectorium.{version}.db`                | `lectorium/databases/lectorium.{version}.db`              |
| User DB       | —                                                  | `user.db` (native) / `lectorium/databases/user.db` (web) |
...
1. Author / publish the new content DB and `config.json` entry through
   **lectorium-mcp** (`catalog.publish`). The new DB is uploaded as
   `/public/db/lectorium.{version}.db` and `/public/config.json` gains a
