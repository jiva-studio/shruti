...
…which flips every such row to `failed`. From then on the user sees a "retry" indicator instead of a permanent "downloading" lock and the next `downloadMedia` call goes through without the `"already-in-progress"` short-circuit. See [`mediaItemRepository.ts`](https://github.com/jiva-studio/lectorium/blob/main/modules/libs/domain/ports/mediaItemRepository.ts) for the port contract.
