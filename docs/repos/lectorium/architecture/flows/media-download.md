...
Force-close, OS kill, or app crash mid-download leaves a row in `downloading` with `localPath: null` and no in-flight transfer. The download store's `hydrate()` (`modules/apps/mobile/lectorium/stores/useDownloadStore.ts`) calls it once before reading `listReady()`:
...
…which flips every such row to `failed`. From then on the user sees a "retry" indicator instead of a permanent "downloading" lock and the next `downloadMedia` call goes through without the `"already-in-progress"` short-circuit. See [`mediaItemRepository.ts`](https://github.com/akdasa-studios/lectorium/blob/main/modules/libs/domain/ports/mediaItemRepository.ts) for the port contract.
