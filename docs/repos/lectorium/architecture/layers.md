...
| **Must NOT import** | `@ports`, `@infra`, `@ui`, `@lectorium`, `@lib/persistence`, `vue`, platform APIs |
...
| **Must NOT import** | `@lib/domain`, `@usecases`, `@infra`, `@ui`, `@lectorium` |
...
| **May import** | `@ports/app`, `@lib/domain` (types + domain services), `@lib/contracts`, `@lib/persistence/*` (row types), `@kit/*` (incl. `@kit/infra`/`@kit/persistence`), and the in-house `@lectorium/plugin-*` packages |
| **Must NOT import** | `@ui`, `@lectorium` (except `@lectorium/plugin-*`), `@usecases`, other `@infra/*` siblings |
| **Subdirectories** | |
...
| `infra/audio/capacitor/` | `IAudioPlayer` wrapping the in-house `@lectorium/plugin-audio-player` plugin — single cross-platform adapter (native + web fallback handled inside the plugin); same single-adapter pattern as `mediaDownloader/plugin/` |
| `infra/mediaDownloader/plugin/` | `IMediaDownloader` over the in-house [`@lectorium/plugin-media-downloader`](../modules/media-downloader.md) plugin (Android WorkManager + OkHttp, iOS background URLSession, Web Cache API) — single adapter for every platform |
...
> submodule and is wired in `lectorium/main.ts`: `idbKv` (the low-level
...
| **Must NOT import** | `@ports`, `@infra`, `@lib/domain`, `@usecases`, `@lectorium/*`, cross-feature UI siblings |
...
```
lectorium/  →  @ui, @infra, @ports, @usecases, @lib/domain, @lib/contracts, @kit
@ui/*       →  @kit/ui (shared primitives), @lib/ui (shared components), lower UI sub-layers only
@infra/*    →  @ports, @lib/domain, @lib/contracts, @lib/persistence, @kit (incl @kit/infra), @lectorium/plugin-*
...
noting: infra may import the in-house `@lectorium/plugin-*` packages (the
Capacitor plugins share the `@lectorium` npm scope). `@lib/ui` is the only
