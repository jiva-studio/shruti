...
- **Purchases store** (`lectorium/stores/usePurchasesStore.ts`) exposes `isSubscribed`. After `purchase()`/`restore()` it calls `useAuthStore().refreshTokens()` so the new `pro` claim lands immediately instead of waiting for natural rotation, and `onCustomerInfoChanged` force-refreshes when RC's view diverges from the JWT tier.
