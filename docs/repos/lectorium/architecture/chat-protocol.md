...
| `X-Lectorium-Region` | proxy-only | Set by the RU proxy, honoured only from trusted source IPs (`_region.py`). Gates PII handling (e.g. free-text feedback is dropped for `region=ru`). Clients never set this. |
