...
worktree-serve-bg: ## Start dev server in background: make worktree-serve-bg ISSUE=42
	...
	@echo "[worktree-serve-bg] Log:  /tmp/shruti-worktree-$(ISSUE).log"
	@cd $(WORKTREE_DIR)/modules/apps/mobile && VITE_PORT=$(WORKTREE_PORT) nohup npm run dev > /tmp/shruti-worktree-$(ISSUE).log 2>&1 &
...
# --- Shruti services: transcriber stack (delegates to modules/tools/Makefile) ---
...
# --- Shruti services: shruti-mcp daemon (delegates to modules/tools/shruti-mcp/Makefile) ---

shruti-mcp-build: ## Build the shruti-mcp binary
	@$(MAKE) -C modules/tools/shruti-mcp build

shruti-mcp-test: ## Run shruti-mcp unit tests
	@$(MAKE) -C modules/tools/shruti-mcp test

shruti-mcp-lint: ## Run staticcheck on shruti-mcp
	@$(MAKE) -C modules/tools/shruti-mcp lint

shruti-mcp-up: ## Start shruti-mcp daemon
	@$(MAKE) -C modules/tools/shruti-mcp up

shruti-mcp-down: ## Stop shruti-mcp daemon
	@$(MAKE) -C modules/tools/shruti-mcp down

shruti-mcp-restart: ## Restart shruti-mcp daemon
	@$(MAKE) -C modules/tools/shruti-mcp restart

shruti-mcp-status: ## Show shruti-mcp daemon status
	@$(MAKE) -C modules/tools/shruti-mcp status
...
stack-up: ## Start the local stack with secrets injected from 1Password (op run; requires op unlocked + a populated "Dev :: App" item)
...
stack-up-noop: ## Start the stack WITHOUT 1Password (op-sourced API keys unset → chat degraded; for offline/CI)
